<template>
	<div class="bg-[var(--color-black1)] relative overflow-hidden">
		<div class="flex max-phoneX:flex-col-reverse">
			<div class="bg-white hidden max-phoneX:flex px-[8.375rem] py-[9rem] pb-0">
			...
			<div class="flex items-center justify-center absolute left-[6.3rem] top-28 max-phoneX:top-0 max-phoneX:relative max-phoneX:left-0 max-phoneX:justify-start max-phoneX:px-[8.375rem] max-phoneX:py-[1.8rem]">
				<div class="flex flex-col gap-4">
					<span class="text-white text-5xl font-mont font-bold leading-tight text-left max-phoneX:text-start" v-html="title"></span>
					<span class="text-white text-xl font-[Arial] font-semibold max-phoneX:hidden" v-html="description"></span>
				</div>
			</div>
		</div>
	</div>
